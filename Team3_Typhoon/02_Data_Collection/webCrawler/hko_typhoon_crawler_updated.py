#!/usr/bin/env python3
"""
HKO Typhoon Signal 8 Web Crawler - Updated Version
==================================================
...
class HKOTyphoonCrawlerUpdated:
    """Updated web crawler for HKO typhoon signal data collection"""
    ...
    def __init__(self, base_dir="/Users/simonwang/Documents/Usage/GCAP3226/Team3_Typhoon/02_Data_Collection/webCrawler"):
        ...
        # Updated HKO URLs (2025)
        self.base_url = "https://www.hko.gov.hk"
        self.warning_url = "https://www.hko.gov.hk/en/wservice/warning/ts.htm"
        ...
        self.api_url = "https://data.weather.gov.hk/weatherAPI/opendata/weather.php?dataType=warnsum&lang=en"
        ...
        self.logger.info("HKO Typhoon Crawler (Updated) initialized")
    ...
    def setup_logging(self):
        ...
        log_file = os.path.join(self.logs_dir, f"hko_crawler_updated_{timestamp}.log")
        
        # Create logger
        self.logger = logging.getLogger('HKOTyphoonCrawlerUpdated')
    ...
    def fetch_api_data(self):
        ...
        try:
            self.log_crawl_activity("API_REQUEST", f"Fetching API data from: {self.api_url}")
            response = self.make_request(self.api_url)
            
            if response:
                data = response.json()
                self.log_crawl_activity("API_SUCCESS", f"Retrieved API data: {len(data)} items")
                
                # Look for Signal 8 data
                signal_8_data = []
                for key, value in data.items():
                    if 'TC8' in str(value) or 'Signal No. 8' in str(value) or 'Gale' in str(value):
                        signal_8_data.append({
                            'timestamp': datetime.now().isoformat(),
                            'key': key,
                            'value': value,
                            'source': 'HKO_API'
                ...
                self.signal_8_announcements.extend(signal_8_data)
                self.log_crawl_activity("API_PARSE", f"Found {len(signal_8_data)} Signal 8 references in API")
                
                return data
            return None
            
        except Exception as e:
            self.log_crawl_activity("API_ERROR", f"Error fetching API data: {str(e)}")
    ...
    def generate_report(self):
        ...
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_path = os.path.join(self.output_dir, f"crawling_report_updated_{timestamp}.md")
            
            with open(report_path, 'w', encoding='utf-8') as report_file:
                report_file.write("# HKO Typhoon Signal 8 Crawling Report (Updated)\n\n")
                report_file.write(f"**Generated:** {datetime.now().isoformat()}\n")
                report_file.write(f"**Crawler:** HKO Typhoon Signal 8 Web Crawler (Updated)\n")
                ...
                for i, announcement in enumerate(self.signal_8_announcements, 1):
                    ...
                    report_file.write(f"- **Source:** {announcement.get('source', 'N/A')}\n\n")
                ...
                for i, weather in enumerate(self.weather_data, 1):
                    ...
                    report_file.write(f"- **Source:** {weather.get('source', 'N/A')}\n\n")
    ...
    def crawl_hko_data(self):
        ...
        self.log_crawl_activity("CRAWL_START", "Starting HKO typhoon data crawling (Updated)")
        self.logger.info("Starting HKO typhoon data crawling (Updated)")
        
        try:
            # 1. Try API first
            self.log_crawl_activity("CRAWL_API", "Fetching data from HKO API")
            ...
            if self.signal_8_announcements:
                self.save_data_to_csv(self.signal_8_announcements, "signal_8_announcements_updated.csv")
                self.save_data_to_json(self.signal_8_announcements, "signal_8_announcements_updated.json")
            
            # Save weather data
            if self.weather_data:
                self.save_data_to_csv(self.weather_data, "weather_data_updated.csv")
                self.save_data_to_json(self.weather_data, "weather_data_updated.json")
            
            # Save crawl log
            self.save_data_to_json(self.crawl_log, "crawl_log_updated.json")
...
def main():
    """Main function to run the updated crawler"""
    print("🌪️ HKO Typhoon Signal 8 Web Crawler (Updated)")
    print("=" * 60)
    ...
    # Initialize updated crawler
    crawler = HKOTyphoonCrawlerUpdated()
    ...
    print("\n🌪️ HKO Typhoon Signal 8 Web Crawler (Updated) - Complete")
